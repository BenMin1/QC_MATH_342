...
This picture plots the data with the x axis representing the first_feature, the y axis representing the second_feature, and the color representing the output as a 1 or a 0. This visual clearly shows the data to be linearly separable.  
...
The slope is -1 and the intercept is + 6. This output is showing that the weight repeats implying that a valid separating line was drawn. 
...
It's not satisfying because the separating line does not go through the middle. This line does not provide a cushion for predictions as if it is a little off, it could cause bad outputs. 
...
Yes. The line is more towards the middle of the wedge. 
...
linear_svm_learning_algorithm = function(Xinput, y_binary, MAX_ITER = 5000, lambda = 0.1){
  #First set SHE = sum(max(0, half - (y - half) (w dot x_i - b)))
  #Second optimize(w_0, b_0, 1/n * SHE + lambda norm(w)^2, max_iter) to minimize SHE
  #Third 
}
```
...
#'
#' Uses euclidean distance to find the closest match in the dataset and predicts based on the data's output.
#'
#' @param Xinput      This is the training data (nxp)
#' @param y_binary    Training data outputs (nx1)
#' @param Xtest       Test matrix (predictions x p)
#' @return            prediction vector of outputs for predictions
...
#expect_equal(my_simple_ols_mod$b_1, as.numeric(coef(lm_mod)[3]), tol = 1e-4)
...
#expect_equal(0, mean(my_simple_ols_mod$e))
```
...
b_0 is the height of a child of heightless parents
b_1 is expected to be positive
RMSE
R^2

How good is this model? How well does it predict? Discuss.

This model lowers the standard deviation to 2.24 inches from 2.52 inches. While this may be statistically significant, it is not practically significant. 
...
Yes. Under this assumption, the slope should be one. This is because height is determined by genes inherited from both of the parents.  

If they were to have the same height and any differences were just random noise with expectation 0, what would the values of beta_0 and beta_1 be?

beta_0 would be the mean height with b_1 being 1. 
...
This is because the children tend towards the population mean and are not purely functions of the parents mean heights. 

Why should this effect be real?

Heights is influenced by more than purely the parents heights. 
