...
You should use the mutate function. 

```{r}
bills_and_payments_and_discounts %>%
  mutate(past_discounts_dummy = if_else(is.na(days_until_discount), 0, 1))
```
...
y_test = f_x(x_train) + rnorm(n_train, 0, sigma)
...
```{r}
n_test = 500
x_train = runif(n_test, x_min, x_max)
...
nodesizes = seq(1,100, by = 10)
oos_pred = c(rep(NA, length(nodesizes)))

for(i in 1:length(nodesizes)){
  mod = randomForest(y_train ~ x_train, ntree = 1, replace = FALSE, samsize = n, nodesize = nodesizes[i])
  predictions = predict(mod, x_test)
  curr_oos_prediction = mean((predictions - y_test)^2)
  oos_pred[i] = curr_oos_prediction
}

plot(nodesizes, oos_pred)
```

Plot the regression tree model g(x) with the optimal node size.

```{r}
plot(nodesizes[which.min(oos_pred)], oos_pred[which.min(oos_pred)])
```

Find the oosRMSE of this optimal-node-size model.

```{r}
oos_pred[which.min(oos_pred)]
```
...
for(i in 1:length(nodesizes)){
  mod_tree = rpart(lettr ~ ., data = letters_train, minsplit = nodesizes[i])
  
  yhat = predict(mod_tree, type = "class")
  
  misclassification_errs[i] = mean(yhat != letters_select$lettr)
}

min_error = which.min(misclassification_errs)
best_node = nodesizes[min_error]
```
...
ggplot(data.frame(nodesize = nodesizes, misclassification_error = misclassification_errs)) + 
  aes(x = nodesize, y = misclassification_errs, misclassification_error = misclassification_error) +
  geom_point() + 
  geom_line()
```
...
```{r}
y_hat_test = rpart(lettr ~ ., data = letters_train, minsplit = 101)
tree_mod_opt = mean(y_hat_test != letters_select$lettr)
```
...
```{r}
confusion_matrix = table(y_hat_test, c(letters_test$lettr))
```
